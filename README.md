...
- `geo` — currently set to downtown Nashville's public coordinates
  (36.1627, -86.7816) as a service-area-business approximation, not a real
  street address. Replace with the actual business location if one exists.
...
- **Logo** (`public/nashville-concrete-solutions.png`, cropped to
  ...
  `src/components/layout/Logo.tsx`) — the favicon set (`public/favicon.ico`,
  `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`,
  `android-chrome-{192,512}.png`, generated by `/tmp/gen_favicons.py`-style
  script, not committed) and the OG image still use a generated "N" monogram
  (the full logo is illegible at 16–32px), now in navy. Swap those to the
  real logo artwork if you'd rather have brand-exact favicons.
